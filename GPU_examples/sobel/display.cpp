...
void filter(unsigned int *output)
{
	...
	if (status == CL_SUCCESS) std::cout << "Profiling Success!" << std::endl;
	if (status == CL_PROFILING_INFO_NOT_AVAILABLE) std::cout << "Error: CL_PROFILING_NOT_AVAIL could not get profile information" << std::endl;
	if (status == CL_INVALID_VALUE) std::cout << "CL_INVALID_VALUE Error: could not get profile information" << std::endl;
	if (status == CL_INVALID_EVENT) std::cout << "CL_INVALID_EVENT Error: could not get profile information" << std::endl;
